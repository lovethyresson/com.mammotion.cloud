...
class BaseDriver extends Homey.Driver {
  ...
  isMatchingModel(device) {
    ...
    const productName = (device.productName || device.deviceName || device.name || '').toLowerCase();
    const productModel = (device.productModel || device.model || '').toLowerCase();
    const categoryKey = (device.categoryKey || '').toLowerCase();
    const series = (device.series || device.productSeries || '').toLowerCase();
    const combined = `${productName} ${productModel} ${categoryKey} ${series}`;
    ...
    session.setHandler('list_devices', async () => {
      ...
      try {
        ...
        devices = allDevices
          ...
          .map((device) => ({
            name: device.nickName || device.deviceName || device.productName || device.name || 'Mammotion Mower',
            data: {
            ...
            settings: {
              model: device.series || device.productSeries || device.productModel || device.model || 'Unknown',
